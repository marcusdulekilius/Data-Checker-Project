    ...
    for username1, top_words1 in user_top_words.items():
        for username2, top_words2 in user_top_words.items():
            if username1 != username2 and (username2, username1) not in similarity_scores:
                ...
                # Ağ yapısını oluşturma
G = nx.Graph()

# Her kullanıcının takipçileri ile ilişkilerini ekleyerek ağ yapısını oluşturma
for user_data in dataset:
 ...
 # Her bir kullanıcının en çok tekrar eden 3 kelimesini tutacak bir sözlük oluşturulması
    user_top_3_words = {}
    for user in dataset:
        user_tweets = user['tweets']
        all_user_words = [tweet.split() for tweet in user_tweets]
        user_words = [word for sublist in all_user_words for word in sublist]
        user_word_counts = Counter(user_words)
        user_top_3_words[user['username']] = user_word_counts.most_common(3)  # Her kullanıcının en çok tekrar eden 3 kelimesi
    ...
    plt.xticks(rotation=45)  
     ...
     # Tüm kullanıcıların en çok tekrar eden 3 kelimesini görselleştirme
    plt.figure(figsize=(12, 8))
    for idx, (username, top_words) in enumerate(user_top_3_words.items()):
        words, counts = zip(*top_words)
        plt.barh([f'{username} - {word}' for word in words], counts, label=f'User {idx + 1}')

    plt.xlabel('Tekrar Sayısı')
    plt.ylabel('Kullanıcılar - Kelimeler')
    plt.title('Kullanıcıların En Çok Tekrar Eden 3 Kelimesi')
    plt.legend()
    ...
    # Ağı çizme
    plt.figure(figsize=(12, 8))
    pos = nx.spring_layout(G)  # Node'ların konumunu belirleme
